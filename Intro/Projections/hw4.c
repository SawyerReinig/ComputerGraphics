...
void display()
{
   ...
   switch (mode)
   {
      ...
      case 0:
         ...
         BeachMat(0.5,0.15,0.33,25,-20,0,0); 
      ...
      case 1:
         ...
         CreateUmbrella(1.2,0.3,1,0.42, 0, -1, -3, 5, 1); 
         BeachMat(-0.6,0.18,0.33,25,-20,0,0);
         BeachMat(0.5,0.16,0.33,25,-20,0,0); 
         BeachMat(1,0.35,1,10,-1,0,0); 
         for(int i = 0; i < 9; i++){
            DrawCurlingWave(t/12,0.3111 * i);
         }      
   ...
   Print("Angle=%d,%d Dim=%.1f FOV=%d   %s",th,ph,dim,fov, text[mode]);
...
int main(int argc,char* argv[])
{
   ...
   //  Create the window
   glutCreateWindow("Sawyer Reinig FP Beach");
#ifdef USEGLEW
   //  Initialize GLEW
